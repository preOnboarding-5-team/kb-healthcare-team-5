...
### 나의 건강점수를 특정 기준과 분석/비교
* 나의 건강점수를 분석하고, 연령대/성별이 동일한 그룹의 데이터와 비교하여 나의 평균점수가 더 낮으면 빨간색, 높으면 파란색, 동일하면 검정색 문구를 표출합니다. 
* 동일 그룹 내에서 나의 평균점수가 상위 0~50%면 '상위 ( 0~50% )', 상위 51%~100%면 '하위 ( 0~50% )' 문구를 표출합니다.
* 나의 건강점수가 10년 후의 예측 건강점수보다 낮으면 빨간색, 높으면 파란색, 동일하면 검정색 문구를 표출합니다. 
* 10년 후의 예상 의료비가 현재보다 많을 것으로 예상되면 빨간색, 적으면 파란색, 동일하면 검정색 문구를 표출합니다. 
